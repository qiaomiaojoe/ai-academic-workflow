...
5. **检索源如实报告。** 方法节必须写明"检索源为 OpenAlex（+ 补充源）"，不得表述成 Scopus / Web of Science 检索。局限节要写明未使用受控词表与商业库，**以及未检索中文数据库**（除非确实人工检索了并有记录）。
...
得到 `records.csv` / `.jsonl`（含 DOI、题名、年份、期刊、作者、被引数、摘要、命中了哪几个块）。记下三个数：**原始命中数、去重后待筛数、摘要覆盖率**——直接抄脚本打印的 `PRISMA · Identification` 那行，不要自己相减。

**PRISMA 的数字必须来自不带 `--cap` 的完整抓取。** `--cap` 是试跑用的，加了它脚本会明确打印"被截断、没抓完"——那种结果只能当样本看，**未抓取的部分不是去重差额**，写进 PRISMA 就是错的。另外 OpenAlex 的 `meta.count` 是估算值，与翻页实际遍历数可能差几条，脚本会在有出入时提示，**以遍历数为准并在方法节说明**。
