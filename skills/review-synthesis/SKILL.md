...
- **输入**：① `01_选题/选题.md`（锚块定型 + RQ + 纳入标准）；② 最终纳入集——`02_文献/系统综述/筛选决定.csv` 中 `final_decision = include` 的记录（无该列时用人工确认过的纳入清单）；③ 这些研究的**全文**（Zotero MCP / 本地 `02_文献/pdf_download/` / NotebookLM，按可用性降级）。
- **输出**：默认 `04_数据/`；`研究发现.md` 是唯一正式产出，中间产物带前缀不覆盖。
...
**一篇一行**，逐篇读全文填抽取表 `04_数据/综合-抽取表.csv`。**逐篇处理是为了防爆上下文，不是汇报节奏——不要每篇停下汇报。**
...
中间产物：`综合-抽取表.csv`、`综合-meta分析.R`/`.py` 与其输出、`综合-森林图.*`、`综合-主题矩阵.md`、`综合-概念矩阵.md`（按型取用），多轮不覆盖（加 `-R2` / `-R3`），记账追加 `04_数据/分析日志.md`。
